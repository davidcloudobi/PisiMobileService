...
namespace PisiMobile.BusinessCore.Integration
{
    public class LogHandler
    {
        ...
        public async Task Invoke(HttpContext context, IOptions<AppSettings> generalSettings, DatabaseContext appDbContext)
        {
            ...
            try
            {
                ...
                var entry = JsonConvert.DeserializeObject<RequestResponseData>(item);
                ...
                if (validateToken)
                {
                    ...
                    var exisingToken = await appDbContext.AccessTokens.FirstOrDefaultAsync(t => t.Token == token && t.UserId == user.Id);
                ...
                finally
                {
                    ...
                    await appDbContext.RequestResponseDatas.AddAsync(entry);
